...
void pc_relative(){
  ...
  char ff[9] = "11111111";
  char localflags[9] = "00000000";
  char pccp[9] = "00000000";
  ...
  alu(ALU_OP_ADD, pcl, dbr, pccp, localflags);
  cp_register(pccp, pcl);

  if(dbr[0]=='1'){
	alu(ALU_OP_ADD_WITH_CARRY, pch, ff, pccp, localflags);
  } else {
	alu(ALU_OP_ADD_WITH_CARRY, pch, zero, pccp, localflags);
  }
  cp_register(pccp, pch);
...
void cpu_6502_ora_imm(){
  cycles = 2;
  char *reg =   edata_imm();
  alu(ALU_OP_OR,acc,reg,acc,flags);
...
void cpu_6502_and_imm(){
  cycles = 2;
  char *reg =   edata_imm();
  alu(ALU_OP_AND,acc,reg,acc,flags);
set_rw2write();
access_memory();
}
...
void cpu_6502_eor_imm(){
  cycles = 2;
  char *reg =   edata_imm();
 alu(ALU_OP_XOR,acc,reg,acc,flags);
...
void cpu_6502_adc_imm(){
  cycles = 2;
  char *reg =   edata_imm();
  alu(ALU_OP_ADD_WITH_CARRY,acc,reg,acc,flags);
...
void cpu_6502_sbc_imm(){
  cycles = 2;
  char *reg =   edata_imm();
 alu(ALU_OP_SUB_WITH_CARRY,acc,reg,acc,flags);
...
void cpu_6502_dec_zp(){
  cycles = 5;
  char *reg =   edata_zp();
  alu(ALU_OP_SUB,reg,"00000001",dbr,NULL);
...
void cpu_6502_dex_imp(){
  cycles = 2;
  alu(ALU_OP_SUB,idx,"00000001",idx,NULL);
  zsflagging(flags,idx);
}
...
void cpu_6502_dey_imp(){
  ...
  cp_register(flags,reg);
  alu(ALU_OP_SUB, idy, "00000001", idy, reg);
  zsflagging(flags,idy);
}
...
void cpu_6502_inc_zp(){
  cycles = 5;
  char *reg =   edata_zp();
  alu(ALU_OP_ADD,reg,"00000001",dbr,NULL);
  zsflagging(flags,dbr);
  set_rw2write();
  access_memory();
...
void cpu_6502_inx_imp(){
  cycles = 2;
  char dflags[REG_WIDTH+1];
  alu(ALU_OP_ADD,idx,"00000001",idx,dflags);
  zsflagging(flags,idx);
}
...
void cpu_6502_iny_imp(){
  cycles = 2;
  char dflags[REG_WIDTH+1];
  alu(ALU_OP_ADD,idy,"00000001",idy,dflags);
  zsflagging(flags,idy);
}
...
void cpu_6502_ror_imp(){
  cycles = 2;
  alu(ALU_OP_ROR,acc,NULL,acc,flags);
