...
VPATH = src/alu:src/util:src/cpu:src/memory:src/test:src/include:src/monitor:src/graphic:src/cpu/c64:src/sound:src/cmdline:tests:src/cia

# printing: how many colums per source code page
COLS=2

tags:	
	cd src && find . -name "*.[chCH]" -print | etags -
...
cia.o : cia.c cia.h memory.o  register.o
	$(CC) $(CFLAGS) -c $<

cia-main : cia-main.c printers.o cpu-util.o mu-code.o decode.o memory.o register.o src-ripper.o alu.o decode.o cpu-util.o flags.o adr-modes.o sdl-driver.o bitmap.o chardev.o parser.o
	@echo -e "\ncompiling cia-main"
	$(CC) $(CFLAGS)  -o cia-main $^ -lreadline `sdl-config --cflags --libs`
...
graphic-main : graphic-main.c cpu.o printers.o cpu-util.o mu-code.o decode.o memory.o register.o src-ripper.o alu.o decode.o cpu-util.o flags.o adr-modes.o sdl-driver.o bitmap.o chardev.o parser.o
	...
	$(CC) $(CFLAGS)  -o graphic-main $^ -lm -lreadline `sdl-config --cflags --libs`
...
test-disco : graphic-main.c cpu.o printers.o cpu-util.o mu-code.o decode.o memory.o register.o alu.o decode.o cpu-util.o flags.o adr-modes.o sdl-driver.o bitmap.o chardev.o parser.o
	cat src/test/disco.test | ./graphic-main --memory sipiu-codebase/disco/disco.mem --source sipiu-codebase/disco/disco.dis > src/test/disco.out 

test-color : graphic-main
	cat src/test/color.test |  ./graphic-main --memory sipiu-codebase/color/color.mem --source sipiu-codebase/color/color.dis > src/test/color.out
